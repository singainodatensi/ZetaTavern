...
【最重要ルール】
- 時系列を保つこと
- 誰が誰とどう出会ったか、なぜ同行・協力・対立しているかを落とさないこと
- 陣営参加、契約、救助、重要な宣言、関係性の転換点は必ず残すこと
- 細かな会話の反復は畳んでよいが、物語の前提は失わないこと
- 未回収の課題、約束、今後向かう先があれば残すこと`;
...
export async function buildSystemInstruction(story, options = {}) {
  ...
  instruction += `以下の条件に当てはまる場合、本文を書く前に必要なローカル参照または検索を行うこと。\n`;
  instruction += `1. 原作キャラクターを新規登場させる時: 所属、陣営、家族、相棒、同行しやすい人物、拠点を確認すること。単独で出すと不自然な人物は、周辺人物の存在を確認し、必要なら自然に同席・同行・話題化すること。\n`;
  instruction += `2. 新しい場所・組織・制度・イベントを扱う時: その作品固有の舞台情報を確認すること。汎用ファンタジー、汎用学園、汎用ラブコメの代用品で埋めず、その作品に固有の地名、学校、商会、騎士団、ユニット、家系、地域勢力、依頼文化、政治構造などを優先すること。\n`;
  instruction += `3. ユーザーが目的を持つ行動を宣言した時: 行動文をそのまま検索語にせず、「次の場面に必要な原作知識は何か」を判断してから参照・検索すること。依頼なら敵・依頼主・地域・制度、ライブなら会場・出演者・ユニット、移動なら目的地の勢力・進行中イベント・主要人物を確認すること。\n`;
  instruction += `4. 原作キャラ同士の関係が展開に影響する時: その人物単体ではなく、周辺人物との関係、距離感、立場、利害、同席しやすさまで確認すること。\n`;
  instruction += `5. 作品固有の空気を出したい時: その場面をその作品らしくするために必要な固有要素が不足しているなら、参照または検索を行うこと。ただ会話するだけ、依頼を受けるだけ、学校で会うだけ、で終わらせず、「その作品である意味」が出る要素を1つ以上混ぜること。\n`;
  instruction += `【参照しなくてよい条件】\n`;
  instruction += `- 既存の検索メモ、キャラクターライブラリ、ロアブックだけで十分に描写できる場合\n`;
...
export async function generateSessionChapterSummary(story, options = {}) {
  const appState = getState();
  const apiKey = appState.apiKey || await getApiKeyFromStorage();
  ...
  if (segments.length === 0) {
    throw new Error('章要約の対象セグメントがありません。');
  }

  const modelName = resolveSessionSummaryModelName(appState);
  const usageAccumulator = createUsageAccumulator('story-summary', modelName);
  const startTurn = Math.min(...segments.map(segment => Number(segment.startTurn || 1)));
  const endTurn = Math.max(...segments.map(segment => Number(segment.endTurn || segment.startTurn || 1)));
  ...
  const basePrompt = String(appState.sessionSummaryPrompt || '').trim() || DEFAULT_SESSION_SUMMARY_PROMPT;
  const summaryPrompt = [
    basePrompt,
    '',
    '【章圧縮モードの追加ルール】',
    ...
    '',
    '【指示】',
    '以下の圧縮要約群を、古い出来事の連続性を保ったまま一つの章要約へ再圧縮してください。',
    '出力は章要約本文のみとし、見出しや箇条書きラベルの再掲は不要です。',
    '',
    '【統合対象の圧縮要約】',
    blocks
  ].join('\n');
  ...
  if (!response.ok) {
    const errorData = await response.json().catch(() => ({}));
    const message = errorData.error?.message || `HTTP status ${response.status}`;
    throw new Error(`章要約API Error: ${message}`);
  }
  ...
  const summaryText = String(extracted?.text || '').trim();
  if (!summaryText) {
    throw new Error('章要約テキストを取得できませんでした。');
  }
